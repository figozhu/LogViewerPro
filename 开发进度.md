...
| 编号 | 任务 | 描述 | 优先级 | 状态 | 备注 |
| --- | --- | --- | --- | --- | --- |
| QA-01 | 单元测试（模板/日志） | 覆盖模板 CRUD、Regex 测试以及日志查询核心流程 | 高 | 已完成 | 新增 Vitest Pinia 单测验证 templateStore/logsStore 行为，确保输入校验与 IPC 交互 |
| QA-02 | Worker 解析验证 | 以样例日志验证 LogIndexer 写入、跳过与进度上报 | 高 | 已完成 | 新增 log-indexer Vitest，用真实 SQLite/日志文件验证插入与 unmatched 统计 |
| QA-03 | 查询逻辑回归 | 覆盖全文检索、过滤、分页、列校验等 QueryBuilder 关键路径 | 高 | 已完成 | 新增 Vitest + query-builder 单元测试，确保 SQL/参数/异常逻辑稳定 |
| QA-04 | 渲染层状态测试 | 覆盖 Pinia Store（偏好、系统日志）行为及 IPC 交互 | 高 | 已完成 | 新增 Vitest Pinia 单测，验证 fetch/update/fetchRecent/setLimit 等关键逻辑 |
...
| 编号 | 任务 | 描述 | 优先级 | 状态 | 备注 |
| --- | --- | --- | --- | --- | --- |
| D-01 | 用户指南 | 覆盖安装、模板管理、索引查询、日志/偏好与 FAQ | 中 | 已完成 | 新增 doc/用户指南.md，详细描述操作步骤与故障排查 |
| D-02 | 架构文档 | 描述 Renderer/Main/Worker 模块与 IPC 协议 | 中 | 已完成 | 新增 doc/架构与API.md，涵盖模块职责、数据流与扩展指引 |
| D-03 | 发布与回滚计划 | 明确构建流程、预发布清单与故障排查 | 中 | 已完成 | 新增 doc/发布与故障排查.md，提供 checklist/常见问题/反馈渠道 |
