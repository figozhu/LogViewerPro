...
- **前端 (Renderer):** **Vue 3 (TypeScript + Composition API)**
- **数据库:** **`better-sqlite3`** (用于在 Worker 线程中进行高性能同步I/O)
- **状态管理:** Pinia (官方推荐的 Vue 状态容器)
- **UI 库:** `vue-virtual-scroller` (用于虚拟滚动)
...
> 架构图:
>
> (Renderer: Vue UI) <-- IPC --> (Main: Window Mgmt)
>
...
- **UI:** 在 Vue 3 中创建一个专门的 `<TemplateManager>` 组件（可作为路由页面或模态框）来承载 CRUD 操作。
...
- **启动流程:**
  ...
  5. Vue 组件 `<LogTable>` 根据 `columns` 状态动态渲染表头和行。
  ...
  9. Vue 组件 `<Sidebar>` 根据 `filterOptions` 动态渲染过滤器。
- **虚拟滚动:**
  - 使用 `vue-virtual-scroller` 的 `<RecycleScroller>` 组件。
  - `item-count` 设置为日志总数（通过 `SELECT COUNT(*)` 获取）。
  - `v-slot="{ item }"` 渲染函数负责渲染单行，并按 `columns` 状态动态渲染每个单元格。
